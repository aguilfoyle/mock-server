...
var GetFloormapToken = {
    ...
    mockRoute: '\/api\/v1\/floor-map-tokens\/site-id\/\\d+$', //a regex for the route, ex. '\/api\/steps\/.*\/users' is a valid route
...
var SaveFloor = {
    name: 'SaveFloor',
    mockRoute: '\/api\/v1\/floors/', //a regex for the route, ex. '\/api\/steps\/.*\/users' is a valid route
    testScope: 'success', //success=200 & a scenario response | notFound=404 | error=500 and there's many more...
    ...
    jsonTemplate: [{
        ...
        saveResult: function () {
            return JSON.stringify({
                result: [schemas.floor],
                count: 1
            });
        }
    }]
};
mocks.push(SaveFloor);
